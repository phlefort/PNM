...
my $cmd ='sed  s/subcase.*/subcase\ \ \ \ \ \ \ \ \ '.($i+$nmin).'/g num.'.$case.'>num.tmp;mv num.tmp num.'.$case.';
sed  s/Beta.*/Beta\ 293\ 0.001801\ 8.31\ '.$vartemp.'/g fluid.'.$case.'>fluid.tmp;cp fluid.tmp fluid.'.$case;
#.';touch data_'.($i+$nmin);
   system($cmd);
...
$cmd = 'rm data_new;touch data_new;mpirun -np '.$nproc.' ./arpovs;cat data_new>>data_'.($i+$nmin).';rm '.$case.'-'.$num.'*.[apv]*;rm *.lgf *.tgz ';
			           system($cmd);
